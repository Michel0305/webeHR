  ...
  <div>
    ...
    >
      ...
      <el-form-item label="加班時數">
        ...
        <el-checkbox v-model="overtimefrmData.lunch" label="中餐" name="lunch">午餐</el-checkbox>
        <el-checkbox v-model="overtimefrmData.dinner" label="晚餐" name="dinner">晚餐</el-checkbox>
      </el-form-item>
      ...
      <el-form-item label="備注信息">
        <el-input v-model="overtimefrmData.remark" type="textarea"/>
      </el-form-item>
      {{ infodata }}
    </el-form>
  </div>
...
export default {
  ...
  methods: {
    getWorkDate() {
      const stWork = Math.min(...this.overtimefrmData.workDate)
      const edWork = Math.max(...this.overtimefrmData.workDate)
      const keyinDays = eformCommon.getDaysofbetween(stWork, edWork)
      this.isSubmit = true
      if (keyinDays > 10) {
    ...
    getWorkHours() {
      const workHours = eformCommon.checkoverworkHours(this.overtimefrmData.startTime, this.overtimefrmData.endTime)
